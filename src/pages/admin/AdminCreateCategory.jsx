...
export default function AdminCreateCategory() {
  ...
  const handleSubmit = async (e) => {
    ...
    if(imageFile){
      categoryData.append("img", imageFile);
    }
    const response = await dispatch(createCategory(categoryData)).unwrap();
  ...
  return (
    ...
    >
      <form
        ...
        className="w-full lg:w-[50%] bg-white rounded-3xl border border-[#E2E8F0] shadow-xl shadow-[#a53860]/5 p-8 flex flex-col items-center gap-y-7"
      >
       ...
       <div className="w-full flex gap-x-3 justify-between">
        ...
        >
         {loading ?  <span className="loader"></span>: 'حفظ المنشأة'} 
          <FaLongArrowAltLeft />
        </button>
         <button
